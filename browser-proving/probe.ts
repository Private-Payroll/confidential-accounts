...
async function main() {
  ...
  const irBytes = new Uint8Array(await (await fetch('/artefacts/zkir/addSigner.bzkir')).arrayBuffer());
  ...
  const proverKey = new Uint8Array(await (await fetch('/artefacts/keys/addSigner.prover')).arrayBuffer());
  const verifierKey = new Uint8Array(await (await fetch('/artefacts/keys/addSigner.verifier')).arrayBuffer());
   ...
   */
  try {
    await provider.prove(new Uint8Array([0, 1, 2, 3]), 'contract:probe/addSigner?vk=0');
