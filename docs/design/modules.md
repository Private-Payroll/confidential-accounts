...
|---|---|
| modules walked | 333 |
| module-to-module import sites | 1083 |
| modules that could not be read | 0 |
| specifiers that resolved to nothing | 0 |
| specifiers naming a real file outside the walked set | 77 |
...
| `src/midnight/ledger.ts` | 2 | 9 | 12 | 44 | 46 | 5 | 22 |
| `src/midnight/vault-ledger.ts` | 2 | 9 | 7 | 12 | 22 | 0 | 32 |
...
| `ConfidentialAccount.assetKeyOf` | `contracts/test/commitments.test.ts` `contracts/test/one-definition.test.ts` `contracts/test/simulator.ts` `scripts/run-preview.ts` `src/midnight/commitments.ts` | 80 |
| `ConfidentialAccount.changeCommitmentOf` | `contracts/test/commitments.test.ts` `contracts/test/one-definition.test.ts` `contracts/test/the-service-layer-meets-the-chain.test.ts` `src/midnight/commitments.ts` `src/midnight/ledger.ts` | 80 |
...
| `ConfidentialAccount.removeSignerPayload` | `contracts/test/one-definition.test.ts` `contracts/test/signer-governance.test.ts` `contracts/test/vault-threshold-recovery.test.ts` `scripts/governance-steps.ts` `scripts/measure-call-cost.ts` `src/midnight/commitments.ts` | 79 |
...
| `ConfidentialAccount.allVaults` | `contracts/test/one-definition.test.ts` `contracts/test/vault-scoping.test.ts` `scripts/diagnose-state.ts` `scripts/prove-compare.ts` `scripts/run-preview.ts` `scripts/sponsor-test.ts` `src/midnight/commitments.ts` | 62 |
| `ConfidentialAccount.noVault` | `contracts/test/one-definition.test.ts` `contracts/test/payout-runs.test.ts` `contracts/test/vault-scoping.test.ts` `contracts/test/vault-threshold-recovery.test.ts` `scripts/governance-steps.ts` `scripts/run-preview.ts` `src/midnight/commitments.ts` `src/midnight/ledger.test.ts` | 81 |
| `ConfidentialAccount.paidMovementOf` | `contracts/test/one-definition.test.ts` `contracts/test/payout-runs.test.ts` `contracts/test/the-payroll-run-meets-the-chain.test.ts` `contracts/test/transcript.test.ts` `contracts/test/vault-client.test.ts` `contracts/test/vault-payout.test.ts` `contracts/test/vault-scoping.test.ts` `contracts/test/vault-threshold-recovery.test.ts` `contracts/test/vault-unshielded.test.ts` `src/midnight/ledger.ts` | 69 |
...
| `ConfidentialAccount.payoutLeaf` | `contracts/test/one-definition.test.ts` `contracts/test/payout-runs.test.ts` `contracts/test/vault-scoping.test.ts` `src/midnight/payout-tree.ts` | 100 |
| `ConfidentialAccount.setVaultThresholdPayload` | `contracts/test/one-definition.test.ts` `contracts/test/vault-registry.test.ts` `contracts/test/vault-scoping.test.ts` `contracts/test/vault-threshold-recovery.test.ts` `scripts/measure-call-cost.ts` `src/midnight/commitments.ts` | 77 |
| `ConfidentialAccount.vacantSlot` | `contracts/test/one-definition.test.ts` `contracts/test/simulator.ts` `contracts/test/what-a-signer-is.test.ts` `scripts/governance-steps.ts` `src/midnight/ledger.test.ts` `src/midnight/ledger.ts` | 70 |
...
| `ConfidentialAccount.slotOf` | `contracts/test/one-definition.test.ts` `contracts/test/signer-governance.test.ts` `contracts/test/simulator.ts` `scripts/governance-steps.ts` | 70 |
| `ConfidentialAccount.amendSigner` | `contracts/test/simulator.ts` `scripts/governance-steps.ts` `scripts/measure-call-cost.ts` `scripts/prove-compare.ts` `scripts/run-preview.ts` `src/midnight/ledger.ts` | 69 |
| `ConfidentialAccount.setThreshold` | `contracts/test/simulator.ts` `scripts/governance-steps.ts` `scripts/measure-call-cost.ts` `src/midnight/ledger.ts` | 68 |
| `ConfidentialAccount.propose` | `contracts/test/simulator.ts` `scripts/cross-contract-spike.ts` `scripts/governance-steps.ts` `scripts/measure-call-cost.ts` `scripts/run-preview.ts` `src/midnight/ledger.ts` `src/web/proving-runner.test.ts` | 70 |
| `ConfidentialAccount.approve` | `contracts/test/simulator.ts` `scripts/governance-steps.ts` `scripts/measure-call-cost.ts` `scripts/run-preview.ts` `src/midnight/job-runner.test.ts` `src/midnight/ledger.ts` `src/midnight/what-a-balance-books.test.ts` | 69 |
| `ConfidentialAccount.cancel` | `contracts/test/simulator.ts` `scripts/governance-steps.ts` `scripts/measure-call-cost.ts` `scripts/run-preview.ts` `src/midnight/ledger.ts` | 68 |
| `ConfidentialAccount.closeExpiredRun` | `contracts/test/simulator.ts` `scripts/measure-call-cost.ts` `src/midnight/ledger.ts` | 67 |
| `ConfidentialAccount.recordPayment` | `contracts/test/simulator.ts` `scripts/measure-call-cost.ts` | 36 |
| `ConfidentialAccount.setVaultThreshold` | `contracts/test/simulator.ts` `scripts/measure-call-cost.ts` `src/midnight/ledger.ts` | 67 |
...
| `Vault.heldCommitmentOf` | `contracts/test/vault-payout.test.ts` `contracts/test/vault-split.test.ts` `src/midnight/vault-ledger.test.ts` | 24 |
| `Vault.noteBlindingOf` | `contracts/test/vault-payout.test.ts` `contracts/test/vault-split.test.ts` `src/midnight/vault-ledger.test.ts` | 24 |
| `Vault.deposit` | `contracts/test/transcript.test.ts` `contracts/test/vault-client.test.ts` `contracts/test/vault-payout.test.ts` `contracts/test/vault-recovery.test.ts` `contracts/test/vault-registry.test.ts` `contracts/test/vault-split.test.ts` `contracts/test/vault-threshold-recovery.test.ts` `contracts/test/vault-unshielded.test.ts` `scripts/chain-probe.ts` `scripts/measure-call-cost.ts` `src/midnight/vault-call-convention.test.ts` `src/midnight/vault-ledger.test.ts` `src/midnight/vault-ledger.ts` | 23 |
| `Vault.depositUnshielded` | `contracts/test/vault-client.test.ts` `contracts/test/vault-unshielded.test.ts` `src/midnight/vault-ledger.test.ts` `src/midnight/vault-ledger.ts` | 15 |
| `Vault.payout` | `contracts/test/transcript.test.ts` `contracts/test/vault-client.test.ts` `contracts/test/vault-payout.test.ts` `contracts/test/vault-recovery.test.ts` `contracts/test/vault-registry.test.ts` `contracts/test/vault-split.test.ts` `contracts/test/vault-threshold-recovery.test.ts` `contracts/test/vault-unshielded.test.ts` `scripts/cross-contract-spike.ts` `scripts/measure-call-cost.ts` `src/midnight/vault-call-convention.test.ts` `src/midnight/vault-ledger.test.ts` `src/midnight/vault-ledger.ts` | 23 |
| `Vault.payoutUnshielded` | `contracts/test/vault-client.test.ts` `contracts/test/vault-unshielded.test.ts` `src/midnight/vault-ledger.test.ts` `src/midnight/vault-ledger.ts` | 15 |
...
| 251 | `SealedStateStore` | `export interface SealedStateStore` |
| 279 | `privateStateKey` | `export const privateStateKey = (base: string, accountId: string): string =>` |
| 314 | `MidnightLedger` | `export class MidnightLedger implements Ledger` |
| 2141 | `UndecodedLedgerField` | `export class UndecodedLedgerField extends Error` |
| 2235 | `MidnightProofSystem` | `export class MidnightProofSystem implements ProofSystem` |
| 2412 | `AuthorityShape` | `export type AuthorityShape = 'anyone' \| 'no-one' \| 'one-key' \| 'committee';` |
| 2418 | `OnChainAuthority` | `export interface OnChainAuthority` |
| 2453 | `AuthorityRead` | `export type AuthorityRead` |
| 2475 | `authorityShapeOf` | `export function authorityShapeOf(` |
| 2495 | `authorityFromContractState` | `export function authorityFromContractState(` |
| 2527 | `ContractStateReader` | `export type ContractStateReader = (address: string) => Promise<unknown>;` |
| 2536 | `readContractAuthority` | `export async function readContractAuthority(` |
| 2572 | `intendedAuthorityValue` | `export function intendedAuthorityValue(` |
| 2626 | `AuthorityVerdict` | `export type AuthorityVerdict = 'agree' \| 'disagree' \| 'unknown';` |
| 2628 | `AuthorityComparison` | `export interface AuthorityComparison` |
| 2663 | `compareAuthority` | `export function compareAuthority(` |
| 2848 | `MaintenanceRefusal` | `export interface MaintenanceRefusal` |
| 2888 | `verifierKeyRefusals` | `export function verifierKeyRefusals(` |
| 2948 | `authorityValueRefusals` | `export function authorityValueRefusals(` |
| 3050 | `requireBuildableAuthority` | `export function requireBuildableAuthority(` |
| 3073 | `MaintenancePlan` | `export type MaintenancePlan` |
| 3110 | `planAuthorityReplacement` | `export function planAuthorityReplacement(` |
| 3200 | `MaintenanceEndStateRecord` | `export interface MaintenanceEndStateRecord` |
| 3237 | `EndStateVerdict` | `export type EndStateVerdict = 'settled' \| 'not-yet' \| 'unexplained' \| 'unknown';` |
| 3239 | `EndStateCheck` | `export interface EndStateCheck` |
| 3254 | `checkEndState` | `export function checkEndState(` |
| 3295 | `MaintenanceSignature` | `export type MaintenanceSignature = AuthorityKey;` |
| 3310 | `MaintenanceUpdateLike` | `export interface MaintenanceUpdateLike` |
| 3317 | `MaintenancePrimitives` | `export interface MaintenancePrimitives` |
| 3333 | `VerifierKeyWrite` | `export interface VerifierKeyWrite` |
| 3360 | `BuiltMaintenanceInstruction` | `export interface BuiltMaintenanceInstruction` |
| 3434 | `buildMaintenanceInstruction` | `export function buildMaintenanceInstruction(` |
| 3533 | `attachMaintenanceSignature` | `export function attachMaintenanceSignature(` |
| 3567 | `signatureProgress` | `export function signatureProgress(` |
| 3620 | `OnChainOperation` | `export interface OnChainOperation` |
| 3627 | `OperationsRead` | `export type OperationsRead` |
| 3638 | `operationsFromContractState` | `export function operationsFromContractState(` |
| 3688 | `VerifierKeyVerdict` | `export type VerifierKeyVerdict = 'agree' \| 'disagree' \| 'unknown';` |
| 3690 | `VerifierKeyComparison` | `export interface VerifierKeyComparison` |
| 3731 | `compareVerifierKeys` | `export function compareVerifierKeys(` |

### `src/midnight/ledger.ts` — what it refuses

| line | kind | message |
|---|---|---|
| 404 | throw | cannot open "[value]": this MidnightLedger was built without deployment credentials. Deploying needs a maintenance authority and somewhere to record the address the chain assigns, and a client that only calls an existing account has no business holding either. |
| 441 | throw | cannot open "[value]": the opening names no founding signer. The account's first seat is the leaf handed to the constructor, and an account deployed without one can never have a signer added — `amendSigner` requires an existing signer, so there would be nobody able to seat the first. |
...
| 524 | throw | cannot open "[value]": a threshold of [value] is not a rule. It is refused here because it would enter our own record of the account as its policy. The chain never sees it: since `C340` the constructor takes no threshold and founds every account at one, raised afterwards through the ordinary approval path. |
| 803 | throw | movementsset |
| 870 | throw | propose succeeded but the account state could not be read back, so it is unknown whether the proposal is open. Approvals gathered against it may be unusable.n looked for id: [value]n state read back: none |
| 878 | throw | propose succeeded but the chain has no open proposal with this id. Approvals gathered against it would be unusable.n looked for id: [value]n ids on chain ([value]): none, |
| 889 | throw | propose succeeded but the chain recorded a different change than the one supplied.n on chain: [value]n supplied: [value]nEvery approval gathered against this proposal would be unusable. |
| 932 | throw | a payroll run must name the vault that will pay it: the vault is folded into the proposal id and recordPayment recomputes the id from the vault it is handed, so a run raised at the no-vault sentinel is one no vault can ever present. |
| 948 | throw | proposeRun succeeded but the chain has no open proposal with this run's id. Approvals gathered against it would be unusable, and no payment could ever match it. |
| 955 | throw | proposeRun succeeded but the chain recorded a different change than the one supplied.n on chain: [value]n supplied: [value]nEvery approval gathered against this run would be unusable. |
| 1210 | throw | a governance round cannot name a vault |
| 1239 | throw | a payroll run needs at least one payee |
| 1242 | throw | this run's window closes at [value] and opens at [value], so no payment could ever fall inside it |
| 1254 | throw | [value] is not a time in seconds — that is the year [value]. Block time is seconds since the Unix epoch, not milliseconds. |
| 1307 | throw | the contract has no state on chain |
| 1317 | throw | adding a signer to a live account needs an approved proposal. Propose the signer, reach the threshold, then add them. M-37. |
| 1357 | throw | the contract has no state on chain |
| 1370 | throw | the threshold must be at least one, and a whole number |
| 1378 | throw | the threshold cannot exceed the [value] signers on this account, or one signer could seat their own. |
| 1426 | throw | a vault threshold of zero would authorise anything |
| 1438 | throw | the contract has no state on chain |
| 1441 | throw | that would leave [value] signers against a threshold of [value], and the account could never approve anything again. |
| 1509 | throw | [value] is thirty-two zero bytes. Supply a signer leaf that is neither thirty-two zero bytes nor the vacancy marker. The contract refuses this value — it answers "that is not a usable signer leaf", in its constructor for a founding leaf and in `amendSigner` for a leaf being seated — because thirty-two zero bytes is what an empty slot reads as, so a seat holding it is a seat nothing can ever prove. |
| 1518 | throw | [value] is the vacancy marker itself. Supply a signer leaf that is neither thirty-two zero bytes nor the vacancy marker. The contract refuses this value — it answers "that is not a usable signer leaf", in its constructor for a founding leaf and in `amendSigner` for a leaf being seated — because the tree uses the marker to mean THIS SLOT IS EMPTY, so seating it makes a slot that is simultaneously taken and free. |
| 1550 | throw | the contract has no state on chain |
| 1553 | throw | cannot [value]: there is no open proposal [value] on this account. It has either settled, been cancelled, or was never raised here. |
| 1572 | throw | cannot [value]: that proposal has [value] of [value] approvals. The contract would reject it. |
| 1606 | throw | account not found on this ledger |
| 1608 | throw | account not found on this ledger |
| 1612 | throw | the state for "[value]" is already sealed at key epoch [value]. Re-sealing over an existing epoch would destroy the only copy under that key. |
| 1631 | throw | committed state could not be retrieved for "[value]" at key epoch [value]. Either the blob is missing (M-73), or the account record and the state store disagree about which viewing key is current, which is what a half-finished rotation looks like (K-4). |
| 1667 | throw | account "[value]" is not deployed on Midnight |
| 1740 | throw | no asset blinding in the private state for "[value]" on this device. It is written when the account is opened and every signer needs the same one — without it this device cannot derive the account's asset key, and so cannot compute or check the change commitment a proposal carries. |
| 1884 | throw | the deployed contract has no circuit "[value]" |
| 2206 | throw | set |
| 2240 | throw | no Compact circuit for "[value]" yet |
| 2241 | throw | not implemented: requires the proof server at |
| 2245 | throw | not implemented: verification happens on chain |
| 2592 | throw | a maintenance authority at threshold [value] is not a committee. Set the threshold to at least one and no more than the number of keys in the committee. A threshold ABOVE the committee size is the unmaintainable state — say { kind: "unmaintainable" } deliberately if that is the intent, rather than reaching it by arithmetic. A threshold BELOW one is WORLD-WRITABLE, not unmaintainable: MEASURED on `@midnightntwrk/ledger-v9@1.0.0-rc.3`, a maintenance update carrying NO SIGNATURES AT ALL is well-formed against an authority at threshold zero, so anybody at all could replace this contract's verifier keys while holding nothing. Committee membership IS still checked — a signature at an out-of-range seat is refused, and so is a wrong signature at a valid seat — what is missing is any requirement to attach one. This refuses rather than comparing the value against the chain. |
| 3057 | throw | this maintenance authority will not be built:n - [[value]] [value]n |
| 3446 | throw | this plan carries no on-chain authority, so there is nothing to say who must sign the update it describes. A `build` plan always carries one; a plan that does not is a defect in this module rather than a fact about the contract, and building anyway would produce an instruction nobody can be told how to sign. |
| 3460 | throw | these verifier-key writes will not be built:n - [[value]] [value]n |
| 3540 | throw | seat [value] is not a seat on the committee that currently maintains [value]: it holds [value] seat(s), numbered 0 to [value]. The signatures on a maintenance update are checked against the CURRENT committee, not the one being installed. |
| 3548 | throw | seat [value] has already signed this update. A second signature at the same index is refused by the chain as a malformed transaction, and attaching it here would waste a submission rather than add a vote. |
| 3556 | throw | this signature does not verify against the key in seat [value] of [value]'s current committee. Either it was made by a different key, or it was made over different data — a signature is bound to (contract address, exact update list, counter), so one collected for another contract, or before the counter moved, is dead. |

### `src/midnight/ledger.ts` — where a width is fixed

| line | site | what fixes it |
|---|---|---|
| 565 | `randomBytes(32)` | a fixed-width random draw |
| 1554 | `.slice(0, 12)` | a fixed-width slice |
| 2483 | `.length === 1` | a width compared |
| 3497 | `.slice(0, 8)` | a fixed-width slice |
| 3743 | `.slice(0, 8)` | a fixed-width slice |

## `src/midnight/vault-ledger.ts`

Tier 2 of the declared set.

- **imports** — `src/core/crypto.ts`, `src/core/ledger.ts`, `src/midnight/circuit-arity.ts`, `src/midnight/ledger.ts`, `src/midnight/payee-address.ts`, `src/midnight/vault-coins.ts`, `src/midnight/vault-contract.ts`, `src/midnight/vault-notes.ts`, `src/midnight/vault-recovery.ts`
- **imported by** — `scripts/deposit-to-vault.ts`, `scripts/fund-vault.ts`, `scripts/open-vault-pool.ts`, `src/midnight/vault-call-convention.test.ts`, `src/midnight/vault-ledger.test.ts`, `src/midnight/vault-pool.ts`, `src/midnight/vault-witness-binding.test.ts`
...
| line | name | as written |
|---|---|---|
| 44 | `VaultPayment` | `export interface VaultPayment` |
| 105 | `VaultPaid` | `export type VaultPaid` |
| 126 | `NotePool` | `export interface NotePool` |
| 155 | `CallPlan` | `export interface CallPlan` |
| 177 | `planCall` | `export const planCall = (` |
| 223 | `VaultChainUnreadable` | `export class VaultChainUnreadable extends Error` |
| 266 | `VaultPoolDisagreesWithChain` | `export class VaultPoolDisagreesWithChain extends Error` |
| 289 | `VaultCannotAfford` | `export class VaultCannotAfford extends Error` |
| 330 | `VaultAlreadyHoldsNotes` | `export class VaultAlreadyHoldsNotes extends Error` |
| 344 | `VaultLedger` | `export class VaultLedger` |
| 1539 | `toNote` | `export const toNote = (` |
| 1543 | `toHex` | `export { toHex }` |

### `src/midnight/vault-ledger.ts` — what it refuses

| line | kind | message |
|---|---|---|
| 532 | throw | the deployed vault has no circuit "[value]" |
| 715 | throw | a run with no payments is not a run |
| 742 | throw | chain-unreadablethe chain could not be read, so nothing has confirmed what this vault holds. THIS IS NOT THE CHAIN SAYING NO and it is still not a reason to pay: an unconfirmed balance is one a run would be sized against |
| 750 | throw | pool-disagreesthe pool and the chain disagree, so this vault has no balance. [value] |
| 762 | throw | notes-do-not-cover |
| 779 | throw | chain-unreadablethe chain could not be read, so nothing has confirmed what this vault holds in public money. THIS IS NOT THE CHAIN SAYING NO, and it is still not a reason to start paying people |
| 796 | throw | public-balance-shortthis run pays [value] of a public token the chain says this vault holds [value] of. A public balance is one number the ledger subtracts from, so the payees before the shortfall settle and the ones after do not |
| 840 | throw | a note of nothing is not a deposit |
| 940 | throw | a deposit of nothing is not a deposit. It moves no money and would seat a colour in this vault's unshielded token set, which retire then refuses to pass — a way to jam a vault's retirement for free, by anybody, since a deposit needs no approval. |
| 1045 | throw | this payee's address is for [value] and this vault is on [value]. The [value] would be accepted either way, so nothing further down would notice. |
| 1143 | throw | the vault paid without asking for a note. The pool cannot be advanced safely; rebuild it from the chain with replayVault before paying again. |
| 1334 | throw | this provider bundle has no queryUnshieldedBalances, so nothing here can say what the chain published for this contractpublic-balance |
| 1345 | throw | the read itself failed: [value]public-balance |
| 1351 | throw | the indexer has no contract action for this address, so it has not published a balance for it yet. That is not a vault holding nothingpublic-balance |
| 1358 | throw | the indexer answered with [value] rather than a list of balances. A shape this client cannot read is our ignorance, not an empty treasurypublic-balance |
| 1376 | throw | one of the indexer's balance rows is not { tokenType: string, balance: bigint }. Skipping it would understate a treasury, so nothing is returnedpublic-balance |
| 1430 | throw | [value] of [value] note(s) in this pool are NOT in the vault's on-chain set, worth [value] between them. The pool claims MORE than the chain will honour, and every one of those notes would be refused at payment time. Unknown to the chain: [value] |
| 1446 | throw | the chain holds [value] note(s) and this pool holds [value]. Every note the pool knows about IS on chain, so the pool claims LESS than the vault holds — a note reached the vault and was never recorded, which is what a crash between a call and the pool write leaves behind. The amount cannot be stated from here, because a commitment discloses nothing. Rebuild the pool from the chain with replayVault. |
| 1469 | throw | the read itself failed: [value] |
| 1479 | throw | the indexer returned no state for this address |
| 1488 | throw | the state did not decode: [value] |
| 1501 | throw | the decoded state has no readable "notes" set. That is not an empty vault — an empty set is a true statement about the vault and a missing one is our ignorance |
...
- **imports** — `src/core/crypto.ts`, `src/midnight/vault-coins.ts`
- **imported by** — `contracts/test/vault-client.test.ts`, `contracts/test/vault-recovery.test.ts`, `scripts/measure-call-cost.ts`, `src/midnight/vault-ledger.test.ts`, `src/midnight/vault-ledger.ts`, `src/midnight/vault-notes.test.ts`, `src/midnight/vault-pool.test.ts`, `src/midnight/vault-pool.ts`
- **outside packages** — *none*
- **platform modules** — *none*
- **circuits named here** — *none*
- **circuits reached** — *through imports, then through what those circuits themselves run* — *none*
...
| line | name | as written |
|---|---|---|
| 53 | `Note` | `export interface Note` |
| 132 | `VaultNotes` | `export interface VaultNotes` |
| 154 | `noteToSpend` | `export const noteToSpend = (notes: Note[], token: Hex, amount: bigint): Note =>` |
| 210 | `afterPayment` | `export const afterPayment = (` |
| 317 | `paymentsFit` | `export const paymentsFit = (` |
| 346 | `afterDeposit` | `export const afterDeposit = (state: VaultNotes, note: Note): VaultNotes =>` |
| 355 | `balanceOf` | `export const balanceOf = (state: VaultNotes, token: Hex): bigint =>` |
| 385 | `witnessesWithoutAPool` | `export const witnessesWithoutAPool = () => (` |
| 396 | `witnessesOver` | `export const witnessesOver = (get: () => VaultNotes, pending: { spending?: Hex }) => (` |

### `src/midnight/vault-notes.ts` — what it refuses

| line | kind | message |
|---|---|---|
| 157 | throw | this vault holds no notes of [value] |
| 163 | throw | no single note covers [value]: the largest is [value] and the pool holds [value] across [value] notes. Merge them first — a payment cannot. |
| 230 | throw | this vault has no note [value] to spend; its pool and the chain disagree |
| 234 | throw | note [value] holds [value] and the payment is [value] |
| 247 | throw | note [value] was spent exactly and the contract emits no change for that, but the call's outputs carry a coin of [value] coming back to the vault. The pool cannot be advanced from two answers about the same money — rebuild it from the chain with replayVault. |
| 264 | throw | note [value] holds [value] and the payment is [value], so the vault kept [value] — but the call's outputs carry no coin coming back to it. Something is being read that is not this payout. The pool is NOT advanced: the change is on chain and a guess at its nonce is a note nobody can spend. |
| 271 | throw | the coin coming back to this vault is worth [value] and the arithmetic says [value] (note [value] of [value], paying [value]). These are two claims about the same money and there is no correct way to pick one. |
| 277 | throw | the coin coming back to this vault is of [value] and the note spent was of [value]. That is not this payout's change. |
| 327 | throw | payment [value] of [value] cannot be made out of this vault: [value] |
| 348 | throw | this vault already holds a note [value] |
| 350 | throw | a note of nothing is not a deposit |
| 387 | throw | this vault was asked which note to spend for [value] on a call that has no note pool. The unshielded circuits move a LEDGER BALANCE and read no witness, so either the contract has changed or this call was routed to the wrong circuit. Nothing is substituted: a note handed over here would be one this path never established the vault holds. |
| 417 | throw | the vault's note [value] is the one to spend for [value], and its position in the chainx27s commitment tree has never been read. It cannot be spent until it is: the transaction would carry a merkle path for a different leaf and the chain would refuse it. The note is safe — an index is not part of a commitment — and reading it costs a rescan of the vaultx27s Zswap state. NOTHING HERE MAY SUBSTITUTE A NUMBER. |
...
- **imports** — `src/core/assets.ts`, `src/core/crypto.ts`, `src/core/types.ts`, `src/midnight/payee-address.ts`, `src/midnight/payout-tree.ts`
- **imported by** — `scripts/transfer-from-vault.ts`, `src/core/a-payroll-run-is-always-private.test.ts`, `src/core/payroll.ts`
...
| line | name | as written |
|---|---|---|
| 57 | `MovementKind` | `export type MovementKind = 'payroll' \| 'transfer';` |
| 70 | `Privacy` | `export type Privacy = 'private' \| 'public';` |
| 83 | `privacyOf` | `export const privacyOf = (p: Payee): Privacy =>` |
| 99 | `entryKindOf` | `export const entryKindOf = (m: MovementKind): EntryKind => m;` |
| 141 | `payrollPayee` | `export function payrollPayee(name: string, payee: Payee): PayeeAddress` |
| 165 | `Transfer` | `export interface Transfer` |
| 210 | `TransferSpec` | `export interface TransferSpec` |
| 262 | `transferOf` | `export function transferOf(spec: TransferSpec): Transfer` |
| 344 | `transferFacts` | `export const transferFacts = (t: Transfer): PaymentFacts => (` |

### `src/core/movement.ts` — what it refuses

| line | kind | message |
|---|---|---|
| 143 | throw | [value] is set up to be paid publicly. A public payment puts the address and the amount on a record anyone can read. An employee is never paid that way, so a payroll run will not accept a public address. To pay a public address, use a one-off transfer. |
| 267 | throw | a transfer amount is a whole number in the asset's smallest unit |
| 269 | throw | a transfer has to move a positive amount |
| 273 | throw | this transfer is set to [value] and the address is a [value] one. An address decides which way it is paid, and this choice cannot change that. Either switch this transfer to [value], or use a [value] address. |
| 292 | throw | this address is on the payroll roster. A public payment puts the address and the amount on a record anyone can read. An employee is never paid that way, so pay them through payroll instead. |
| 305 | throw | give this transfer a reference, so it can be recognised later |
...
*No width here matches the shapes above.* A width this module works to may
arrive in a variable, and this list sees only a number or a capitalised constant.
<!-- GENERATED:END id="modules" body="3d8d95f791a97441" -->
