...
|---|---|
| modules walked | 497 |
| module-to-module import sites | 2042 |
...
| call sites carrying a literal circuit name | 500 |
...
|---|---|---|---|---|---|---|---|
| `src/midnight/payout-tree.ts` | 1 | 3 | 34 | 14 | 13 | 0 | 1 |
| `src/midnight/run-keys.ts` | 1 | 1 | 9 | 8 | 4 | 0 | 0 |
| `src/midnight/commitments.ts` | 1 | 3 | 23 | 2 | 3 | 0 | 12 |
| `src/core/signer-leaf.ts` | 1 | 2 | 7 | 9 | 2 | 1 | 0 |
| `src/core/crypto.ts` | 1 | 0 | 170 | 28 | 2 | 7 | 0 |
...
| `src/midnight/run-status.ts` | 2 | 3 | 9 | 12 | 2 | 0 | 0 |
...
|---|---|---|
| `ConfidentialAccount.signerPublicKey` | `contracts/test/one-definition.test.ts` `contracts/test/simulator.ts` `contracts/test/what-a-signer-is.test.ts` `scripts/diagnose-state.ts` `scripts/governance-steps.ts` `scripts/prove-compare.ts` `scripts/run-preview.ts` `src/midnight/commitments.ts` | 199 |
...
| `ConfidentialAccount.assetKeyOf` | `contracts/test/commitments.test.ts` `contracts/test/one-definition.test.ts` `contracts/test/simulator.ts` `scripts/run-preview.ts` `src/midnight/commitments.ts` | 196 |
| `ConfidentialAccount.changeCommitmentOf` | `contracts/test/commitments.test.ts` `contracts/test/one-definition.test.ts` `contracts/test/the-service-layer-meets-the-chain.test.ts` `src/midnight/commitments.ts` `src/midnight/ledger.ts` | 196 |
...
| `ConfidentialAccount.removeSignerPayload` | `contracts/test/one-definition.test.ts` `contracts/test/signer-governance.test.ts` `contracts/test/vault-threshold-recovery.test.ts` `scripts/governance-steps.ts` `scripts/measure-call-cost.ts` `src/midnight/commitments.ts` | 193 |
...
| `ConfidentialAccount.allVaults` | `contracts/test/one-definition.test.ts` `contracts/test/vault-scoping.test.ts` `scripts/diagnose-state.ts` `scripts/prove-compare.ts` `scripts/run-preview.ts` `scripts/sponsor-test.ts` `src/midnight/commitments.ts` | 177 |
| `ConfidentialAccount.noVault` | `contracts/test/one-definition.test.ts` `contracts/test/payout-runs.test.ts` `contracts/test/vault-scoping.test.ts` `contracts/test/vault-threshold-recovery.test.ts` `scripts/governance-steps.ts` `scripts/run-preview.ts` `src/midnight/commitments.ts` `src/midnight/ledger.test.ts` | 197 |
| `ConfidentialAccount.paidMovementOf` | `contracts/test/a-retry-lives-on-the-leg-it-retries.test.ts` `contracts/test/nobody-is-paid-twice-by-a-restart.test.ts` `contracts/test/one-definition.test.ts` `contracts/test/payout-runs.test.ts` `contracts/test/the-payroll-run-meets-the-chain.test.ts` `contracts/test/transcript.test.ts` `contracts/test/vault-client.test.ts` `contracts/test/vault-payout.test.ts` `contracts/test/vault-scoping.test.ts` `contracts/test/vault-threshold-recovery.test.ts` `contracts/test/vault-unshielded.test.ts` `src/midnight/ledger.ts` | 181 |
...
| `ConfidentialAccount.payoutLeaf` | `contracts/test/one-definition.test.ts` `contracts/test/payout-runs.test.ts` `contracts/test/vault-scoping.test.ts` `src/midnight/payout-tree.ts` | 202 |
| `ConfidentialAccount.setVaultThresholdPayload` | `contracts/test/one-definition.test.ts` `contracts/test/vault-registry.test.ts` `contracts/test/vault-scoping.test.ts` `contracts/test/vault-threshold-recovery.test.ts` `scripts/measure-call-cost.ts` `src/midnight/commitments.ts` | 191 |
| `ConfidentialAccount.vacantSlot` | `contracts/test/one-definition.test.ts` `contracts/test/simulator.ts` `contracts/test/what-a-signer-is.test.ts` `scripts/governance-steps.ts` `src/midnight/ledger.test.ts` `src/midnight/ledger.ts` | 182 |
| `ConfidentialAccount.adoptVaultPayload` | `contracts/test/one-definition.test.ts` `contracts/test/vault-registry.test.ts` `scripts/measure-call-cost.ts` | 31 |
...
| `ConfidentialAccount.slotOf` | `contracts/test/one-definition.test.ts` `contracts/test/signer-governance.test.ts` `contracts/test/simulator.ts` `scripts/governance-steps.ts` | 34 |
| `ConfidentialAccount.amendSigner` | `contracts/test/simulator.ts` `scripts/governance-steps.ts` `scripts/measure-call-cost.ts` `scripts/prove-compare.ts` `scripts/run-preview.ts` | 33 |
| `ConfidentialAccount.setThreshold` | `contracts/test/simulator.ts` `scripts/governance-steps.ts` `scripts/measure-call-cost.ts` | 32 |
| `ConfidentialAccount.propose` | `contracts/test/a-run-raised-and-approved-from-the-page.test.ts` `contracts/test/simulator.ts` `scripts/cross-contract-spike.ts` `scripts/governance-steps.ts` `scripts/measure-call-cost.ts` `scripts/pay-from-vault.ts` `scripts/pay-privately-from-vault.ts` `scripts/run-preview.ts` `src/core/a-proposal-sent-from-a-device.test.ts` `src/server/index.ts` `src/server/two-devices-act-on-one-proposal.test.ts` `src/web/governed-call-builder.test.ts` `src/web/governed-call-builder.ts` `src/web/governed-call-on-device.test.ts` `src/web/governed-call-on-device.ts` `src/web/proving-runner.test.ts` | 76 |
| `ConfidentialAccount.approve` | `contracts/test/a-run-raised-and-approved-from-the-page.test.ts` `contracts/test/simulator.ts` `scripts/governance-steps.ts` `scripts/measure-call-cost.ts` `scripts/pay-from-vault.ts` `scripts/pay-privately-from-vault.ts` `scripts/run-preview.ts` `src/core/a-proposal-sent-from-a-device.test.ts` `src/midnight/job-runner.test.ts` `src/midnight/what-a-balance-books.test.ts` `src/server/two-devices-act-on-one-proposal.test.ts` `src/web/governed-call-builder.test.ts` `src/web/governed-call-builder.ts` `src/web/governed-call-on-device.test.ts` `src/web/governed-call-on-device.ts` | 60 |
| `ConfidentialAccount.cancel` | `contracts/test/simulator.ts` `scripts/governance-steps.ts` `scripts/measure-call-cost.ts` `scripts/run-preview.ts` | 32 |
| `ConfidentialAccount.closeExpiredRun` | `contracts/test/simulator.ts` `scripts/measure-call-cost.ts` | 30 |
| `ConfidentialAccount.recordPayment` | `contracts/test/simulator.ts` `scripts/measure-call-cost.ts` | 136 |
| `ConfidentialAccount.setVaultThreshold` | `contracts/test/simulator.ts` `scripts/measure-call-cost.ts` | 30 |
| `ConfidentialAccount.adopt` | `contracts/test/simulator.ts` `scripts/measure-call-cost.ts` | 30 |
| `ConfidentialAccount.retireVault` | **NONE** | 3 |
| `Vault.payoutDetails` | `contracts/test/a-deposit-made-on-the-device.test.ts` `contracts/test/a-vault-rebuilt-with-us-gone.test.ts` `contracts/test/transcript.test.ts` `contracts/test/vault-client.test.ts` `contracts/test/vault-payout.test.ts` `contracts/test/vault-recovery.test.ts` `contracts/test/vault-registry.test.ts` `contracts/test/vault-split.test.ts` `contracts/test/vault-threshold-recovery.test.ts` `scripts/measure-call-cost.ts` | 119 |
| `Vault.unshieldedPayoutDetails` | `contracts/test/vault-client.test.ts` `scripts/pay-from-vault-rules.test.ts` | 113 |
| `Vault.heldCommitmentOf` | `contracts/test/vault-payout.test.ts` `contracts/test/vault-split.test.ts` `src/midnight/vault-ledger.test.ts` | 120 |
| `Vault.noteBlindingOf` | `contracts/test/vault-payout.test.ts` `contracts/test/vault-split.test.ts` `src/midnight/vault-ledger.test.ts` | 120 |
| `Vault.deposit` | `contracts/test/a-deposit-made-on-the-device.test.ts` `contracts/test/a-vault-rebuilt-with-us-gone.test.ts` `contracts/test/transcript.test.ts` `contracts/test/vault-client.test.ts` `contracts/test/vault-payout.test.ts` `contracts/test/vault-recovery.test.ts` `contracts/test/vault-registry.test.ts` `contracts/test/vault-split.test.ts` `contracts/test/vault-threshold-recovery.test.ts` `contracts/test/vault-unshielded.test.ts` `scripts/chain-probe.ts` `scripts/measure-call-cost.ts` `src/midnight/vault-call-convention.test.ts` `src/midnight/vault-ledger.test.ts` `src/midnight/vault-ledger.ts` `src/web/vault-builder.ts` | 119 |
| `Vault.depositUnshielded` | `contracts/test/vault-client.test.ts` `contracts/test/vault-unshielded.test.ts` `src/midnight/vault-ledger.test.ts` `src/midnight/vault-ledger.ts` | 113 |
| `Vault.payout` | `contracts/test/a-deposit-made-on-the-device.test.ts` `contracts/test/a-vault-rebuilt-with-us-gone.test.ts` `contracts/test/transcript.test.ts` `contracts/test/vault-client.test.ts` `contracts/test/vault-payout.test.ts` `contracts/test/vault-recovery.test.ts` `contracts/test/vault-registry.test.ts` `contracts/test/vault-split.test.ts` `contracts/test/vault-threshold-recovery.test.ts` `contracts/test/vault-unshielded.test.ts` `scripts/cross-contract-spike.ts` `scripts/measure-call-cost.ts` `src/midnight/vault-call-convention.test.ts` `src/midnight/vault-ledger.test.ts` `src/midnight/vault-ledger.ts` `src/web/vault-builder.ts` | 119 |
| `Vault.payoutUnshielded` | `contracts/test/vault-client.test.ts` `contracts/test/vault-unshielded.test.ts` `src/midnight/vault-ledger.test.ts` `src/midnight/vault-ledger.ts` | 113 |
...
- **imported by** — `contracts/test/a-deposit-made-on-the-device.test.ts`, `contracts/test/a-private-payment-from-the-page.test.ts`, `contracts/test/a-retry-lives-on-the-leg-it-retries.test.ts`, `contracts/test/a-run-carries-its-own-material.test.ts`, `contracts/test/a-vault-rebuilt-with-us-gone.test.ts`, `contracts/test/approvals.test.ts`, `contracts/test/nobody-is-paid-twice-by-a-restart.test.ts`, `contracts/test/one-definition.test.ts`, `contracts/test/payout-runs.test.ts`, `contracts/test/run-keys.test.ts`, `contracts/test/run-status.test.ts`, `contracts/test/the-payroll-run-meets-the-chain.test.ts`, `contracts/test/transcript.test.ts`, `contracts/test/vault-client.test.ts`, `contracts/test/vault-payout.test.ts`, `contracts/test/vault-recovery.test.ts`, `contracts/test/vault-registry.test.ts`, `contracts/test/vault-scoping.test.ts`, `contracts/test/vault-split.test.ts`, `contracts/test/vault-threshold-recovery.test.ts`, `contracts/test/vault-unshielded.test.ts`, `scripts/measure-call-cost.ts`, `scripts/pay-from-vault-rules.ts`, `src/core/a-payroll-run-is-always-private.test.ts`, `src/core/movement.ts`, `src/core/payroll.ts`, `src/core/types.ts`, `src/midnight/private-payment-wire.test.ts`, `src/midnight/private-payment-wire.ts`, `src/midnight/run-material.ts`, `src/midnight/vault-details.ts`, `src/server/index.ts`, `src/standalone/main.tsx`, `src/testing/vault-details.ts`
...
- **imported by** — `contracts/test/a-leg-is-raised-again-as-itself.test.ts`, `contracts/test/a-payroll-the-vault-cannot-pay-is-not-raised.test.ts`, `contracts/test/a-retry-lives-on-the-leg-it-retries.test.ts`, `contracts/test/a-run-carries-its-own-material.test.ts`, `contracts/test/commitments.test.ts`, `contracts/test/nobody-is-paid-twice-by-a-restart.test.ts`, `contracts/test/one-definition.test.ts`, `contracts/test/the-payroll-run-meets-the-chain.test.ts`, `contracts/test/the-service-layer-meets-the-chain.test.ts`, `contracts/test/what-a-signer-is.test.ts`, `scripts/deploy-preview.ts`, `src/core/a-proposal-sent-from-a-device.test.ts`, `src/core/nobody-is-paid-twice-by-a-retyped-month.test.ts`, `src/core/one-writer-for-a-proposal.test.ts`, `src/midnight/ledger.test.ts`, `src/midnight/ledger.ts`, `src/midnight/the-key-reaches-the-circuit.test.ts`, `src/midnight/the-secret-comes-from-the-keyring.test.ts`, `src/server/two-devices-act-on-one-proposal.test.ts`, `src/web/the-page-renders.test.tsx`, `src/wiring/chain.ts`, `src/wiring/one-wiring-point.test.ts`, `src/wiring/selection.ts`
...
- **imported by** — `contracts/test/a-company-vault-from-the-page.test.ts`, `contracts/test/a-deposit-made-on-the-device.test.ts`, `contracts/test/a-leg-is-raised-again-as-itself.test.ts`, `contracts/test/a-payroll-the-vault-cannot-pay-is-not-raised.test.ts`, `contracts/test/a-private-payment-from-the-page.test.ts`, `contracts/test/a-retry-lives-on-the-leg-it-retries.test.ts`, `contracts/test/a-run-carries-its-own-material.test.ts`, `contracts/test/a-vault-rebuilt-with-us-gone.test.ts`, `contracts/test/approvals.test.ts`, `contracts/test/commitments.test.ts`, `contracts/test/nobody-is-paid-twice-by-a-restart.test.ts`, `contracts/test/one-definition.test.ts`, `contracts/test/payout-runs.test.ts`, `contracts/test/run-keys.test.ts`, `contracts/test/run-status.test.ts`, `contracts/test/the-payroll-run-meets-the-chain.test.ts`, `contracts/test/the-service-layer-meets-the-chain.test.ts`, `contracts/test/transcript.test.ts`, `contracts/test/vault-client.test.ts`, `contracts/test/vault-payout.test.ts`, `contracts/test/vault-recovery.test.ts`, `contracts/test/vault-registry.test.ts`, `contracts/test/vault-scoping.test.ts`, `contracts/test/vault-split.test.ts`, `contracts/test/vault-threshold-recovery.test.ts`, `contracts/test/vault-unshielded.test.ts`, `contracts/test/what-a-signer-is.test.ts`, `scripts/deploy-preview.ts`, `scripts/deposit-to-vault.test.ts`, `scripts/deposit-to-vault.ts`, `scripts/fund-vault.ts`, `scripts/measure-call-cost.ts`, `scripts/pay-from-vault-rules.test.ts`, `scripts/pay-from-vault-rules.ts`, `scripts/pay-from-vault.ts`, `scripts/pay-privately-from-vault.ts`, `scripts/pay-privately-rules.test.ts`, `scripts/pay-privately-rules.ts`, `scripts/preview-signers.test.ts`, `scripts/preview-signers.ts`, `scripts/reconcile-vault-pool-rules.test.ts`, `scripts/reconcile-vault-pool-rules.ts`, `scripts/reconcile-vault-pool.ts`, `scripts/record-a-notes-transaction.ts`, `scripts/transfer-from-vault.test.ts`, `scripts/transfer-from-vault.ts`, `scripts/vault-journal.test.ts`, `scripts/vault-pool-file.test.ts`, `src/core/a-payroll-run-is-always-private.test.ts`, `src/core/a-proposal-sent-from-a-device.test.ts`, `src/core/a-run-is-not-a-governance-round.test.ts`, `src/core/a-vault-must-hold-what-a-round-pays.test.ts`, `src/core/a-vault-s-own-threshold.test.ts`, `src/core/account.ts`, `src/core/challenges.ts`, `src/core/core.test.ts`, `src/core/create-company.test.ts`, `src/core/crypto.test.ts`, `src/core/demo.ts`, `src/core/identity.ts`, `src/core/invite-handover.ts`, `src/core/jobs.ts`, `src/core/ledger-token.test.ts`, `src/core/ledger.ts`, `src/core/nobody-is-paid-twice-by-a-retyped-month.test.ts`, `src/core/one-pending-employee-does-not-refuse-the-run.test.ts`, `src/core/one-writer-for-a-proposal.test.ts`, `src/core/payroll.ts`, `src/core/payslip-key-derive.ts`, `src/core/payslip-key.test.ts`, `src/core/payslip-key.ts`, `src/core/plugins.ts`, `src/core/principal.test.ts`, `src/core/principal.ts`, `src/core/sealed-records.test.ts`, `src/core/sealed-records.ts`, `src/core/sessions.ts`, `src/core/signer-leaf.ts`, `src/core/store-file.test.ts`, `src/core/store-file.ts`, `src/core/store.ts`, `src/core/the-threshold-is-the-chain-s.test.ts`, `src/core/types.ts`, `src/core/wallet-identity.ts`, `src/core/wallet-unlock.test.ts`, `src/core/which-seat-is-yours.test.ts`, `src/db/vault-records.test.ts`, `src/midnight/commitments.ts`, `src/midnight/company-nonce-secret.test.ts`, `src/midnight/company-nonce-secret.ts`, `src/midnight/deposit-nonce.test.ts`, `src/midnight/deposit-nonce.ts`, `src/midnight/job-runner.test.ts`, `src/midnight/job-runner.ts`, `src/midnight/ledger.test.ts`, `src/midnight/ledger.ts`, `src/midnight/note-index-against-the-chain.test.ts`, `src/midnight/note-index.test.ts`, `src/midnight/note-index.ts`, `src/midnight/payee-address.ts`, `src/midnight/payout-tree.ts`, `src/midnight/private-payment-wire.test.ts`, `src/midnight/private-payment-wire.ts`, `src/midnight/private-state-addressing.test.ts`, `src/midnight/rebuild-from-records.test.ts`, `src/midnight/rebuild-from-records.ts`, `src/midnight/run-keys.ts`, `src/midnight/run-material.ts`, `src/midnight/run-skips.ts`, `src/midnight/run-status.ts`, `src/midnight/sealed-record-wire.test.ts`, `src/midnight/sealed-record-wire.ts`, `src/midnight/sealed-store.test.ts`, `src/midnight/sealed-store.ts`, `src/midnight/the-key-reaches-the-circuit.test.ts`, `src/midnight/the-secret-comes-from-the-keyring.test.ts`, `src/midnight/vault-coins.test.ts`, `src/midnight/vault-coins.ts`, `src/midnight/vault-holdings.test.ts`, `src/midnight/vault-holdings.ts`, `src/midnight/vault-journal.test.ts`, `src/midnight/vault-journal.ts`, `src/midnight/vault-ledger.test.ts`, `src/midnight/vault-ledger.ts`, `src/midnight/vault-notes.test.ts`, `src/midnight/vault-notes.ts`, `src/midnight/vault-pool.test.ts`, `src/midnight/vault-pool.ts`, `src/midnight/vault-recovery.ts`, `src/server/approval-signature.test.ts`, `src/server/company-vaults.ts`, `src/server/index.ts`, `src/server/invitations.test.ts`, `src/server/self-payee.test.ts`, `src/server/two-devices-act-on-one-proposal.test.ts`, `src/server/vault-chain.ts`, `src/server/vault-records-authority.test.ts`, `src/server/vault-records-authority.ts`, `src/server/vault-records-route.test.ts`, `src/server/vault-records-route.ts`, `src/standalone/main.tsx`, `src/testing/payees.ts`, `src/web/App.tsx`, `src/web/GovernedCallControls.tsx`, `src/web/Join.tsx`, `src/web/PayoutPanel.tsx`, `src/web/VaultPanel.tsx`, `src/web/accept-seat.test.ts`, `src/web/accept-seat.ts`, `src/web/accepted-address.ts`, `src/web/after-a-company-is-unlocked.test.tsx`, `src/web/connector-wallet.ts`, `src/web/deposit-on-device.test.ts`, `src/web/deposit-on-device.ts`, `src/web/governed-call-builder.ts`, `src/web/http-sealed-pool-store.ts`, `src/web/keyring.ts`, `src/web/private-state.test.ts`, `src/web/private-state.ts`, `src/web/seat-repair.test.ts`, `src/web/seat-repair.ts`, `src/web/vault-builder.ts`, `src/web/vault-operation.test.ts`, `src/web/vault-operation.ts`, `src/web/vault-page-doors.ts`, `src/web/wallet-balance.ts`, `src/web/wallet-unlock.ts`, `src/wiring/a-chain-selection-needs-marked-records.test.ts`, `src/wiring/a-service-can-read-a-vault.test.ts`, `src/wiring/chain.ts`
...
- **imported by** — `contracts/test/a-private-payment-from-the-page.test.ts`, `contracts/test/vault-client.test.ts`, `contracts/test/vault-payout.test.ts`, `scripts/pay-from-vault-rules.ts`, `scripts/pay-from-vault.ts`, `scripts/pay-privately-from-vault.ts`, `scripts/pay-privately-rules.ts`, `scripts/transfer-from-vault.test.ts`, `scripts/transfer-from-vault.ts`, `src/core/a-payroll-run-is-always-private.test.ts`, `src/core/core.test.ts`, `src/core/movement.ts`, `src/core/one-pending-employee-does-not-refuse-the-run.test.ts`, `src/core/payroll.ts`, `src/core/payslip-key.test.ts`, `src/core/principal.ts`, `src/core/types.ts`, `src/core/wallet-payee.ts`, `src/midnight/payee-address.test.ts`, `src/midnight/payout-tree.ts`, `src/midnight/vault-holdings.ts`, `src/midnight/vault-ledger.ts`, `src/server/index.ts`, `src/testing/payees.ts`, `src/web/vault-builder.ts`
...
- **imports** — `src/core/crypto.ts`
- **imported by** — `src/core/payslip-key.ts`, `src/midnight/company-nonce-secret.test.ts`, `src/web/App.tsx`, `src/web/Join.tsx`
...
- **circuits reached** — *through imports, then through what those circuits themselves run* — `ConfidentialAccount.allVaults`, `ConfidentialAccount.assetKeyOf`, `ConfidentialAccount.changeCommitmentOf`, `ConfidentialAccount.noVault`, `ConfidentialAccount.paidMovementOf`, `ConfidentialAccount.proposalIdOf`, `ConfidentialAccount.removeSignerPayload`, `ConfidentialAccount.runPayload`, `ConfidentialAccount.setThresholdPayload`, `ConfidentialAccount.setVaultThresholdPayload`, `ConfidentialAccount.signerAddPayload`, `ConfidentialAccount.signerLeaf`, `ConfidentialAccount.signerPublicKey`, `ConfidentialAccount.vacantSlot`

### `src/midnight/ledger.ts` — exported surface

| line | name | as written |
|---|---|---|
| 103 | `PreparedStep` | `export type PreparedStep` |
| 150 | `PreparedCall` | `export interface PreparedCall` |
| 176 | `MidnightConfig` | `export interface MidnightConfig` |
| 214 | `FeeSponsor` | `export interface FeeSponsor` |
| 303 | `SealedStateStore` | `export interface SealedStateStore` |
| 331 | `privateStateKey` | `export const privateStateKey = (base: string, accountId: string): string =>` |
| 386 | `CIRCUITS_THAT_READ_NO_WITNESS` | `export { CIRCUITS_THAT_READ_NO_WITNESS }` |
| 420 | `CIRCUIT_FOR_STEP` | `export const CIRCUIT_FOR_STEP: Record<PreparedStep['kind'], string>` |
| 434 | `MidnightLedger` | `export class MidnightLedger implements Ledger` |
| 2383 | `UndecodedLedgerField` | `export class UndecodedLedgerField extends Error` |
| 2477 | `MidnightProofSystem` | `export class MidnightProofSystem implements ProofSystem` |
| 2654 | `AuthorityShape` | `export type AuthorityShape = 'anyone' \| 'no-one' \| 'one-key' \| 'committee';` |
| 2660 | `OnChainAuthority` | `export interface OnChainAuthority` |
| 2695 | `AuthorityRead` | `export type AuthorityRead` |
| 2717 | `authorityShapeOf` | `export function authorityShapeOf(` |
| 2737 | `authorityFromContractState` | `export function authorityFromContractState(` |
| 2769 | `ContractStateReader` | `export type ContractStateReader = (address: string) => Promise<unknown>;` |
| 2778 | `readContractAuthority` | `export async function readContractAuthority(` |
| 2814 | `intendedAuthorityValue` | `export function intendedAuthorityValue(` |
| 2868 | `AuthorityVerdict` | `export type AuthorityVerdict = 'agree' \| 'disagree' \| 'unknown';` |
| 2870 | `AuthorityComparison` | `export interface AuthorityComparison` |
| 2905 | `compareAuthority` | `export function compareAuthority(` |
| 3090 | `MaintenanceRefusal` | `export interface MaintenanceRefusal` |
...
| line | kind | message |
|---|---|---|
| 524 | throw | cannot open "[value]": this MidnightLedger was built without deployment credentials. Deploying needs a maintenance authority and somewhere to record the address the chain assigns, and a client that only calls an existing account has no business holding either. |
| 561 | throw | cannot open "[value]": the opening names no founding signer. The account's first seat is the leaf handed to the constructor, and an account deployed without one can never have a signer added — `amendSigner` requires an existing signer, so there would be nobody able to seat the first. |
| 589 | throw | cannot open "[value]": the opening names [value] founding signers and this path can seat exactly one. The constructor creates the founder's seat; every seat after it is `amendSigner`, which requires an existing signer to call it AND, since the constructor stopped taking a threshold, an approved proposal behind it — so the rest are proposed, approved and seated by the founder from their own device, and there is no screen for that yet. Open with the founding signer alone. |
| 644 | throw | cannot open "[value]": a threshold of [value] is not a rule. It is refused here because it would enter our own record of the account as its policy. The chain never sees it: since `C340` the constructor takes no threshold and founds every account at one, raised afterwards through the ordinary approval path. |
| 929 | throw | movementsset |
| 996 | throw | propose succeeded but the account state could not be read back, so it is unknown whether the proposal is open. Approvals gathered against it may be unusable.n looked for id: [value]n state read back: none |
| 1004 | throw | propose succeeded but the chain has no open proposal with this id. Approvals gathered against it would be unusable.n looked for id: [value]n ids on chain ([value]): none, |
| 1015 | throw | propose succeeded but the chain recorded a different change than the one supplied.n on chain: [value]n supplied: [value]nEvery approval gathered against this proposal would be unusable. |
| 1058 | throw | a payroll run must name the vault that will pay it: the vault is folded into the proposal id and recordPayment recomputes the id from the vault it is handed, so a run raised at the no-vault sentinel is one no vault can ever present. |
| 1074 | throw | proposeRun succeeded but the chain has no open proposal with this run's id. Approvals gathered against it would be unusable, and no payment could ever match it. |
| 1081 | throw | proposeRun succeeded but the chain recorded a different change than the one supplied.n on chain: [value]n supplied: [value]nEvery approval gathered against this run would be unusable. |
| 1345 | throw | a governance round cannot name a vault |
| 1374 | throw | a payroll run needs at least one payee |
| 1377 | throw | this run's window closes at [value] and opens at [value], so no payment could ever fall inside it |
| 1389 | throw | [value] is not a time in seconds — that is the year [value]. Block time is seconds since the Unix epoch, not milliseconds. |
| 1456 | throw | the contract has no state on chain |
| 1466 | throw | adding a signer to a live account needs an approved proposal. Propose the signer, reach the threshold, then add them. M-37. |
| 1506 | throw | the contract has no state on chain |
| 1519 | throw | the threshold must be at least one, and a whole number |
| 1527 | throw | the threshold cannot exceed the [value] signers on this account, or one signer could seat their own. |
| 1575 | throw | a vault threshold of zero would authorise anything |
| 1587 | throw | the contract has no state on chain |
| 1590 | throw | that would leave [value] signers against a threshold of [value], and the account could never approve anything again. |
| 1658 | throw | [value] is thirty-two zero bytes. Supply a signer leaf that is neither thirty-two zero bytes nor the vacancy marker. The contract refuses this value — it answers "that is not a usable signer leaf", in its constructor for a founding leaf and in `amendSigner` for a leaf being seated — because thirty-two zero bytes is what an empty slot reads as, so a seat holding it is a seat nothing can ever prove. |
| 1667 | throw | [value] is the vacancy marker itself. Supply a signer leaf that is neither thirty-two zero bytes nor the vacancy marker. The contract refuses this value — it answers "that is not a usable signer leaf", in its constructor for a founding leaf and in `amendSigner` for a leaf being seated — because the tree uses the marker to mean THIS SLOT IS EMPTY, so seating it makes a slot that is simultaneously taken and free. |
| 1699 | throw | the contract has no state on chain |
| 1702 | throw | cannot [value]: there is no open proposal [value] on this account. It has either settled, been cancelled, or was never raised here. |
| 1721 | throw | cannot [value]: that proposal has [value] of [value] approvals. The contract would reject it. |
| 1755 | throw | account not found on this ledger |
| 1757 | throw | account not found on this ledger |
| 1761 | throw | the state for "[value]" is already sealed at key epoch [value]. Re-sealing over an existing epoch would destroy the only copy under that key. |
| 1780 | throw | committed state could not be retrieved for "[value]" at key epoch [value]. Either the blob is missing (M-73), or the account record and the state store disagree about which viewing key is current, which is what a half-finished rotation looks like (K-4). |
| 1823 | throw | account "[value]" is not deployed on Midnight |
| 1896 | throw | no asset blinding in the private state for "[value]" on this device. It is written when the account is opened and every signer needs the same one — without it this device cannot derive the account's asset key, and so cannot compute or check the change commitment a proposal carries. |
| 2126 | throw | the deployed contract has no circuit "[value]" |
| 2448 | throw | set |
| 2482 | throw | no Compact circuit for "[value]" yet |
| 2483 | throw | not implemented: requires the proof server at |
...
- **imports** — `src/core/crypto.ts`, `src/core/ledger.ts`, `src/midnight/circuit-arity.ts`, `src/midnight/deposit-nonce.ts`, `src/midnight/fee-seat.ts`, `src/midnight/ledger.ts`, `src/midnight/note-index.ts`, `src/midnight/payee-address.ts`, `src/midnight/vault-coins.ts`, `src/midnight/vault-contract.ts`, `src/midnight/vault-ledger-shape.ts`, `src/midnight/vault-notes.ts`, `src/midnight/vault-pool.ts`, `src/midnight/vault-recovery.ts`
- **imported by** — `scripts/deposit-to-vault.ts`, `scripts/fund-vault.ts`, `scripts/open-vault-pool.ts`, `scripts/pay-from-vault-rules.test.ts`, `scripts/pay-from-vault-rules.ts`, `scripts/pay-from-vault.ts`, `scripts/pay-privately-from-vault.ts`, `src/midnight/deposit-nonce.test.ts`, `src/midnight/deposit-nonce.ts`, `src/midnight/fee-seat.test.ts`, `src/midnight/note-index.test.ts`, `src/midnight/note-index.ts`, `src/midnight/vault-call-convention.test.ts`, `src/midnight/vault-holdings.test.ts`, `src/midnight/vault-holdings.ts`, `src/midnight/vault-journal.ts`, `src/midnight/vault-ledger.test.ts`, `src/midnight/vault-pool.ts`, `src/midnight/vault-witness-binding.test.ts`, `src/wiring/chain.ts`
...
- **circuits reached** — *through imports, then through what those circuits themselves run* — `ConfidentialAccount.allVaults`, `ConfidentialAccount.assetKeyOf`, `ConfidentialAccount.changeCommitmentOf`, `ConfidentialAccount.noVault`, `ConfidentialAccount.paidMovementOf`, `ConfidentialAccount.payoutLeaf`, `ConfidentialAccount.proposalIdOf`, `ConfidentialAccount.recordPayment`, `ConfidentialAccount.removeSignerPayload`, `ConfidentialAccount.runPayload`, `ConfidentialAccount.setThresholdPayload`, `ConfidentialAccount.setVaultThresholdPayload`, `ConfidentialAccount.signerAddPayload`, `ConfidentialAccount.signerLeaf`, `ConfidentialAccount.signerPublicKey`, `ConfidentialAccount.vacantSlot`, `Vault.deposit`, `Vault.depositUnshielded`, `Vault.heldCommitmentOf`, `Vault.noteBlindingOf`, `Vault.payout`, `Vault.payoutDetails`, `Vault.payoutUnshielded`, `Vault.unshieldedPayoutDetails`

### `src/midnight/vault-ledger.ts` — exported surface

| line | name | as written |
|---|---|---|
| 56 | `VaultPayment` | `export interface VaultPayment` |
| 117 | `VaultPaid` | `export type VaultPaid` |
| 145 | `VaultDeposited` | `export interface VaultDeposited extends TxRef` |
| 173 | `PoolVersion` | `export interface PoolVersion` |
| 179 | `LoadedNotes` | `export interface LoadedNotes extends VaultNotes` |
| 203 | `NotePool` | `export interface NotePool` |
| 260 | `PaymentAttempt` | `export interface PaymentAttempt` |
| 282 | `PaymentJournal` | `export interface PaymentJournal` |
| 293 | `noPaymentJournal` | `export const noPaymentJournal = (): PaymentJournal => (` |
| 317 | `DepositAttempt` | `export interface DepositAttempt` |
| 334 | `DepositJournal` | `export interface DepositJournal` |
| 348 | `noDepositJournal` | `export const noDepositJournal = (): DepositJournal => (` |
| 367 | `CallPlan` | `export interface CallPlan` |
| 389 | `planCall` | `export const planCall = (` |
| 435 | `VaultChainUnreadable` | `export class VaultChainUnreadable extends Error` |
| 478 | `VaultPoolDisagreesWithChain` | `export class VaultPoolDisagreesWithChain extends Error` |
| 501 | `VaultCannotAfford` | `export class VaultCannotAfford extends Error` |
| 542 | `VaultAlreadyHoldsNotes` | `export class VaultAlreadyHoldsNotes extends Error` |
| 556 | `VaultLedger` | `export class VaultLedger` |
| 2185 | `toNote` | `export const toNote = (` |
| 2189 | `toHex` | `export { toHex }` |

### `src/midnight/vault-ledger.ts` — what it refuses

| line | kind | message |
|---|---|---|
| 295 | throw | a private payment writes down the note it is about to spend and the amount before the money moves, and this ledger was given nowhere to write it. Nothing is proved or paid. Construct the ledger with a payment journal -- the sealed one the payment door keeps beside the pool -- and pay again. |
| 350 | throw | a private deposit writes down the coin it is about to create before the money moves, and this ledger was given nowhere to write it. Nothing is proved or deposited. Construct the ledger with a deposit journal -- the sealed one the deposit door keeps beside the pool -- and deposit again. |
| 731 | throw | [value] could not be recorded in this vault's note pool: another writer filed the next version first on all [value] attempts, each of which read the pool again and applied this change to what it held. **Nothing has been written and nothing has been erased.**nThis matters because the money has already moved on chain and the pool is the only record of what a note IS — the chain publishes commitments, which disclose nothing. So the vault now holds money this machine cannot name, and a payment that would reach it is refused.nStop writing this pool from anywhere else, then rebuild it from the chain and the versions this pool has been written at: reconcileVaultPool in src/midnight/vault-recovery.ts. Losing this race five times running is not three writers being busy, so find what else is writing before running anything that spends. |
| 790 | throw | the vault's note [value] changed or left the pool while its place in the chainx27s commitment tree was being read. Nothing is proved or paid. Pay again, and the note will be chosen and read afresh. |
| 883 | throw | the deployed vault has no circuit "[value]" |
| 1090 | throw | a run with no payments is not a run |
| 1117 | throw | chain-unreadablethe chain could not be read, so nothing has confirmed what this vault holds. THIS IS NOT THE CHAIN SAYING NO and it is still not a reason to pay: an unconfirmed balance is one a run would be sized against |
| 1125 | throw | pool-disagreesthe pool and the chain disagree, so this vault has no balance. [value] |
| 1137 | throw | notes-do-not-cover |
| 1154 | throw | chain-unreadablethe chain could not be read, so nothing has confirmed what this vault holds in public money. THIS IS NOT THE CHAIN SAYING NO, and it is still not a reason to start paying people |
| 1171 | throw | public-balance-shortthis run pays [value] of a public token the chain says this vault holds [value] of. A public balance is one number the ledger subtracts from, so the payees before the shortfall settle and the ones after do not |
| 1227 | throw | a note of nothing is not a deposit |
| 1519 | throw | a deposit of nothing is not a deposit. It moves no money and would seat a colour in this vault's unshielded token set, which retire then refuses to pass — a way to jam a vault's retirement for free, by anybody, since a deposit needs no approval. |
| 1596 | throw | this payee's address is for [value] and this vault is on [value]. The [value] would be accepted either way, so nothing further down would notice. |
| 1673 | throw | a private payment spends a note, and a note is spent by its place in the chainx27s commitment tree, which is read from the chain just before the payment. No source of the chainx27s events was given, so nothing is proved or paid. Pass the indexerx27s events to payout and pay again. |
| 1724 | throw | the vault paid without asking for a note. The pool cannot be advanced safely; rebuild it from the chain with replayVault before paying again. |
| 1966 | throw | this provider bundle has no queryUnshieldedBalances, so nothing here can say what the chain published for this contractpublic-balance |
| 1977 | throw | the read itself failed: [value]public-balance |
| 1983 | throw | the indexer has no contract action for this address, so it has not published a balance for it yet. That is not a vault holding nothingpublic-balance |
| 1990 | throw | the indexer answered with [value] rather than a list of balances. A shape this client cannot read is our ignorance, not an empty treasurypublic-balance |
| 2008 | throw | one of the indexer's balance rows is not { tokenType: string, balance: bigint }. Skipping it would understate a treasury, so nothing is returnedpublic-balance |
| 2062 | throw | [value] of [value] note(s) in this pool are NOT in the vault's on-chain set, worth [value] between them. The pool claims MORE than the chain will honour, and every one of those notes would be refused at payment time. Unknown to the chain: [value] |
| 2078 | throw | the chain holds [value] note(s) and this pool holds [value]. Every note the pool knows about IS on chain, so the pool claims LESS than the vault holds — a note reached the vault and was never recorded, which is what a crash between a call and the pool write leaves behind. The amount cannot be stated from here, because a commitment discloses nothing. Rebuild the pool from the chain with replayVault. |
| 2101 | throw | the read itself failed: [value] |
| 2111 | throw | the indexer returned no state for this address |
| 2133 | throw | the state did not decode: [value] |
| 2146 | throw | the decoded state has no readable "notes" set. That is not an empty vault — an empty set is a true statement about the vault and a missing one is our ignorance |
...
- **imports** — `src/core/crypto.ts`, `src/midnight/note-index.ts`, `src/midnight/vault-coins.ts`, `src/midnight/vault-note-deposit.ts`, `src/midnight/vault-recovery.ts`
- **imported by** — `contracts/test/a-deposit-made-on-the-device.test.ts`, `contracts/test/a-private-payment-from-the-page.test.ts`, `contracts/test/a-vault-rebuilt-with-us-gone.test.ts`, `contracts/test/vault-client.test.ts`, `contracts/test/vault-recovery.test.ts`, `scripts/measure-call-cost.ts`, `scripts/pay-from-vault.ts`, `scripts/pay-privately-from-vault.ts`, `scripts/pay-privately-rules.test.ts`, `scripts/pay-privately-rules.ts`, `scripts/reconcile-vault-pool-rules.test.ts`, `scripts/reconcile-vault-pool-rules.ts`, `scripts/vault-pool-file.test.ts`, `src/midnight/note-index.test.ts`, `src/midnight/note-index.ts`, `src/midnight/vault-holdings.test.ts`, `src/midnight/vault-ledger.test.ts`, `src/midnight/vault-ledger.ts`, `src/midnight/vault-note-deposit.ts`, `src/midnight/vault-notes.test.ts`, `src/midnight/vault-pool.test.ts`, `src/midnight/vault-pool.ts`, `src/web/vault-builder.ts`, `src/web/vault-operation.ts`
- **outside packages** — *none*
- **platform modules** — *none*
- **circuits named here** — *none*
- **circuits reached** — *through imports, then through what those circuits themselves run* — `ConfidentialAccount.allVaults`, `ConfidentialAccount.assetKeyOf`, `ConfidentialAccount.changeCommitmentOf`, `ConfidentialAccount.noVault`, `ConfidentialAccount.paidMovementOf`, `ConfidentialAccount.payoutLeaf`, `ConfidentialAccount.proposalIdOf`, `ConfidentialAccount.recordPayment`, `ConfidentialAccount.removeSignerPayload`, `ConfidentialAccount.runPayload`, `ConfidentialAccount.setThresholdPayload`, `ConfidentialAccount.setVaultThresholdPayload`, `ConfidentialAccount.signerAddPayload`, `ConfidentialAccount.signerLeaf`, `ConfidentialAccount.signerPublicKey`, `ConfidentialAccount.vacantSlot`, `Vault.deposit`, `Vault.depositUnshielded`, `Vault.heldCommitmentOf`, `Vault.noteBlindingOf`, `Vault.payout`, `Vault.payoutDetails`, `Vault.payoutUnshielded`, `Vault.unshieldedPayoutDetails`
...
| line | name | as written |
|---|---|---|
| 56 | `Note` | `export interface Note` |
| 153 | `VaultNotes` | `export interface VaultNotes` |
| 180 | `aPaymentCanSpend` | `export const aPaymentCanSpend = (note: Note): boolean =>` |
| 223 | `smallestNoteCovering` | `export const smallestNoteCovering = (notes: readonly Note[], token: Hex, amount: bigint): Note \| undefined =>` |
| 231 | `NoteChoice` | `export type NoteChoice` |
| 264 | `choosingANoteToSpend` | `export const choosingANoteToSpend = (notes: readonly Note[], token: Hex, amount: bigint): NoteChoice =>` |
| 290 | `noteToSpend` | `export const noteToSpend = (notes: readonly Note[], token: Hex, amount: bigint): Note =>` |
| 351 | `afterPayment` | `export const afterPayment = (` |
| 458 | `paymentsFit` | `export const paymentsFit = (` |
| 502 | `afterDeposit` | `export { afterDeposit }` |
| 511 | `withIndexRead` | `export const withIndexRead = (state: VaultNotes, nonce: Hex, index: ChainReadIndex): VaultNotes =>` |
| 525 | `balanceOf` | `export const balanceOf = (state: VaultNotes, token: Hex): bigint =>` |
| 555 | `witnessesWithoutAPool` | `export const witnessesWithoutAPool = () => (` |
| 566 | `witnessesOver` | `export const witnessesOver = (get: () => VaultNotes, pending: { spending?: Hex }) => (` |

### `src/midnight/vault-notes.ts` — what it refuses

| line | kind | message |
|---|---|---|
| 294 | throw | this vault holds no notes of [value] |
| 297 | throw | no single note covers [value]: the largest is [value] and the pool holds [value] across [value] notes. Merge them first — a payment cannot. |
| 302 | throw | no note this vault can spend covers [value]. [value]: [value]. [value] which transaction created it with a hash a spend can read, so a payment cannot read its place in the chain's commitment tree. The money is still on chain and still the vault's. Name the transaction that paid it in to recordCreatingTransaction, or rebuild the pool so the chain is asked, and pay again. |
| 371 | throw | this vault has no note [value] to spend; its pool and the chain disagree |
| 375 | throw | note [value] holds [value] and the payment is [value] |
| 388 | throw | note [value] was spent exactly and the contract emits no change for that, but the call's outputs carry a coin of [value] coming back to the vault. The pool cannot be advanced from two answers about the same money — rebuild it from the chain with replayVault. |
| 405 | throw | note [value] holds [value] and the payment is [value], so the vault kept [value] — but the call's outputs carry no coin coming back to it. Something is being read that is not this payout. The pool is NOT advanced: the change is on chain and a guess at its nonce is a note nobody can spend. |
| 412 | throw | the coin coming back to this vault is worth [value] and the arithmetic says [value] (note [value] of [value], paying [value]). These are two claims about the same money and there is no correct way to pick one. |
| 418 | throw | the coin coming back to this vault is of [value] and the note spent was of [value]. That is not this payout's change. |
| 476 | throw | payment [value] of [value] cannot be made out of this vault: [value] |
| 514 | throw | this vault's pool has no note [value], so there is nothing to record an index against. The pool may have moved on since the note was chosen; choose again. |
| 557 | throw | this vault was asked which note to spend for [value] on a call that has no note pool. The unshielded circuits move a LEDGER BALANCE and read no witness, so either the contract has changed or this call was routed to the wrong circuit. Nothing is substituted: a note handed over here would be one this path never established the vault holds. |
| 589 | throw | the vault's note [value] is the one to spend for [value], and its position in the chainx27s commitment tree has not been read for this call. It cannot be spent until it is: the transaction would carry a merkle path for a different leaf. The note is safe, because an index is not part of a commitment. Its index is read from the events of the transaction that created it, which a private payment does before it calls. NOTHING HERE MAY SUBSTITUTE A NUMBER. |

### `src/midnight/vault-notes.ts` — where a width is fixed

| line | site | what fixes it |
|---|---|---|
| 304 | `.length === 1` | a width compared |
| 305 | `.length === 1` | a width compared |
...
- **imports** — `src/core/crypto.ts`
- **imported by** — `contracts/test/a-deposit-made-on-the-device.test.ts`, `contracts/test/a-vault-rebuilt-with-us-gone.test.ts`, `contracts/test/vault-client.test.ts`, `contracts/test/vault-payout.test.ts`, `contracts/test/vault-recovery.test.ts`, `src/midnight/rebuild-from-records.test.ts`, `src/midnight/rebuild-from-records.ts`, `src/midnight/vault-coins.test.ts`, `src/midnight/vault-journal.ts`, `src/midnight/vault-ledger.ts`, `src/midnight/vault-notes.ts`, `src/midnight/vault-recovery.ts`, `src/web/vault-builder.ts`
- **outside packages** — *none*
- **platform modules** — *none*
- **circuits named here** — *none*
- **circuits reached** — *through imports, then through what those circuits themselves run* — *none*
...
- **imported by** — `scripts/pay-from-vault-rules.test.ts`, `scripts/pay-from-vault.ts`, `scripts/pay-privately-from-vault.ts`, `scripts/transfer-from-vault.test.ts`, `scripts/transfer-from-vault.ts`, `src/core/a-payroll-run-is-always-private.test.ts`, `src/core/ledger-token.test.ts`, `src/core/payroll.ts`
...
- **imported by** — `contracts/test/a-retry-lives-on-the-leg-it-retries.test.ts`, `contracts/test/a-run-carries-its-own-material.test.ts`, `contracts/test/run-status.test.ts`, `src/core/core.test.ts`, `src/core/payroll.ts`, `src/midnight/private-payment-wire.ts`, `src/server/index.ts`, `src/standalone/main.tsx`, `src/web/App.tsx`
- **outside packages** — *none*
- **platform modules** — *none*
- **circuits named here** — *none*
- **circuits reached** — *through imports, then through what those circuits themselves run* — *none*
...
*No width here matches the shapes above.* A width this module works to may
arrive in a variable, and this list sees only a number or a capitalised constant.
<!-- GENERATED:END id="modules" body="fa47fb504d9556e3" -->
