 ...
 * and the account's record of that leaf. Every step is the compiled contract's
...
import { payeeAddress, recipientOf } from '../midnight/payee-address.js';
import { isNetworkName } from '../midnight/network.js';
import type { DetailsOf, PayoutLeafInput } from '../midnight/payout-tree.js';
...
export interface PayslipCircuits {
  /** The vault's details commitment for a shielded payee. */
  details: DetailsOf;
...
export function movementOfPayslip(circuits: PayslipCircuits, p: PayslipPayment): Hex {
  ...
  const payee = payeeAddress(p.paidTo, network);
  const details = toHex(circuits.details(
...
export const contractCircuits = async (): Promise<PayslipCircuits> => {
  ...
  return { details: (await vaultDetailsOf()).shielded, leafOf: payoutLeafOf, movementOf: paidMovementOfLeaf };
};
