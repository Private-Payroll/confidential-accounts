 ...
 * This chooses words on a screen, never where money goes: every payment is
 * built from an address decoded in full, checksum and network, by the service
 * and by the device that builds it. The full decode is not done here because it
 * would put the platform's WebAssembly on the page.
 */
