#!/usr/bin/env node
/**
 * MUTATING THE TWO SYSTEMS PI4a DELETED. `docs/NEXT.md` PI4a.
 *
...
const SUITES = [
  'src/server/deleted-systems.test.ts',
  'src/core/wallet-sign-in.test.ts',
];
const OUT = join(ROOT, 'logs', 'mutate-deleted-systems');

/** file, what it breaks, the exact text, its replacement, the tests that must die. */
const MUTATIONS = [
  {
    id: 1,
    binding: 'THE RECOVERY FLOW STAYS DELETED — a disabled path is one somebody re-enables',
    file: 'src/server/index.ts',
    says: 'somebody puts a recovery route back, which is the exact failure this round is '
      + 'written against',
    from: "app.get('/api/me/sessions', authed, wrap(async (req, res) => {",
    to: "app.post('/api/auth/recover/challenge', wrap(async (_q, s) => { s.json({ ok: true }); }));\n"
      + "app.get('/api/me/sessions', authed, wrap(async (req, res) => {",
    kills: ['THE THREE RECOVERY ROUTES ANSWER 404 — they are deleted, not disabled'],
  },
  {
    id: 2,
    binding: 'THE DEVICE-ENVELOPE ROUTES STAY DELETED',
    file: 'src/server/index.ts',
    says: 'a device route comes back, so the envelope has a door again',
    from: "app.get('/api/me/sessions', authed, wrap(async (req, res) => {",
    to: "app.get('/api/me/devices', authed, wrap(async (_q, s) => { s.json({ devices: [] }); }));\n"
      + "app.get('/api/me/sessions', authed, wrap(async (req, res) => {",
    kills: ['EVERY DEVICE ROUTE ANSWERS 404, INCLUDING THE UPGRADE THAT BUILT THE ENVELOPE'],
  },
  {
    id: 3,
    binding: 'NO SECOND HALF CREEPS BACK INTO THE KEY ROUTE',
    file: 'src/server/index.ts',
    says: 'the envelope field returns in the answer, which is how a deleted shape comes '
      + 'back without any route being re-added',
    from: '  res.json({\n    keyBundle: u.keyBundle,',
    to: '  res.json({\n    keyBundle: u.keyBundle, bundleKey: null,',
    kills: ['AND THE KEY ROUTE HANDS BACK NO BUNDLE KEY — there is no second half now'],
  },
  {
    id: 4,
    binding: 'WHAT REPLACED THE ENVELOPE ACTUALLY WORKS — revoking a session',
    file: 'src/core/identity.ts',
    says: 'ending one session by id reports success and ends nothing, so "removing a '
      + 'device" becomes a revocation that runs and does nothing',
    from: '  async endSession(userId: string, id: string): Promise<boolean> {',
    to: '  async endSession(userId: string, id: string): Promise<boolean> {\n'
      + '    if (userId && id) return true;',
    kills: ['REMOVING A DEVICE IS REVOKING ITS SESSION, AND THAT STILL WORKS'],
  },
  {
    id: 5,
    binding: 'THE CHALLENGE STORE WAS NOT TAKEN WITH THE RECOVERY IT ALSO SERVED',
    file: 'src/core/wallet-identity.ts',
    says: 'the wallet sign-in nonce stops coming from the challenge store — which is '
      + 'exactly what would have broken had challenges.ts gone with the recovery flow',
    from: '    const { challenge, expiresAt } = await this.challenges.issue(handle);',
    to: '    const challenge = toHex(randomBytes(32));\n'
      + "    const expiresAt = new Date(0).toISOString();",
    /* NAMES ONLY THE TEST THAT ACTUALLY DIES. The first draft also named
     * 'A NONCE THIS DEPLOYMENT NEVER ISSUED IS REFUSED', and the harness
     * reported it as EXPECTED TO DIE AND DID NOT — correctly: the mutation
     * still hands out a nonce the store never recorded, so consuming an
     * unissued one fails either way. A kills list is a claim about which
     * assertion is load-bearing, and an inaccurate one is noise in the report
     * somebody later learns to skim. */
    kills: ['A REPLAYED SIGN-IN IS NOT A SIGN-IN — the second use of a nonce is refused'],
  },
];
...
const reportAt = (process.argv.find(a => a.startsWith('--report=')) ?? '').slice(9)
  || join(ROOT, 'logs', 'REPORT-MUTATE-DELETED-SYSTEMS.txt');
...
say(`MUTATING THE TWO DELETED SYSTEMS  —  ${new Date().toISOString()}`);
