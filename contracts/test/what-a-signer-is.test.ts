 ...
 * approve, and on an M-of-N account enough of them means nobody can move the
 * money.
 *
 ...
 *
 * `contracts/test/one-definition.test.ts` says it in its own header, and it is
 * a standing limitation: a mirror whose two sides both end in `pureCircuits`
 * pins the ADAPTER and cannot see what the circuit computes. Two mutations of
 * these very circuits SURVIVED against a green mirror.
 *
 ...
 * file dies.
 *
 * **WHAT IT STILL CANNOT SEE, SAID HERE RATHER THAN CLAIMED AWAY** — an
 * earlier sentence in this header claimed otherwise and was corrected: the
 * circuit BODY. `requireSigner` calls the same `signerLeaf` and
 * `signerPublicKey` that `pureCircuits` exports and that `MidnightCommitments`
 * wraps, so a change to either body moves both sides and this file stays green
 * — which is exactly what happened when the contract's `signerLeaf` was moved
 * and nothing anywhere went red. What the circuit computes is pinned by hand
 * in `contracts/test/commitments.test.ts`. What THIS file holds is the
 * adapter's arguments and encoding, and that a value the product wrote is
 * findable from the device's own witnesses.
 *
 ...
 * `signingSecret`. **No product path writes it there.** When this paragraph
 * was first written the one writer of that field was `src/midnight/ledger.ts`,
 * from an injected `deployer()` whose only implementation in the tree was a
 * script's deterministic seed. **That writer was then removed entirely** — the
 * deploy layer holds no signing material at all now — so the field has NO
 * product writer rather than a wrong one, and the gap is still open. This file
 * hands the same 32 bytes to both sides on purpose, which is what the wiring
 * has to do and does not yet.
 *
 * ── AND THE FOUNDING-SEAT BLOCK BELOW ────────────────────────────────────
 *
 * The block at the end of this file is here rather than in `account.test.ts`
 * for the reason this whole file exists: it is about WHICH LEAF A SEAT IS, and
 * it proves it by ACTING through the real circuits rather than by comparing
 * two derivations.
 */
...
describe('what a signer’s public identity is', () => {
  it('SEATS THE LEAF THE PRODUCT WRITES AND THEN ACTS WITH IT, in circuit', async () => {
     ...
     * makes. The invitee is seated below through an approved round — there is
     * no bootstrap window left, so that is how a real account's signers arrive
     * after the founder's. */
  ...
  it('REFUSES the leaf the writers used to make — the ed25519 public key', async () => {
    /*
     ...
     * `amendSigner` never looks at what a leaf means, and there is no path that
     * seats one without a proposal behind it. */
    await sim.seatLeaf(fromHex(oldWay), [FOUNDER], 12);

    sim.as(sim.applying(INVITEE, change(0n, 12)));
    /* **THE MESSAGE IS READ, NOT THE FACT OF A THROW.** `rejects.toThrow()`
     * passes for a `TypeError` from a renamed helper, and a broad pattern
     * passes for any refusal mentioning a path. The witness that refuses is
     * `contracts/src/witnesses.ts`, by name. */
  ...
  it('SEATS WHAT `AccountService.create` WROTE, THROUGH THE WRITER AND NOT THE HELPER', async () => {
    /*
     * **THE WRITER ITSELF, BECAUSE THE HELPER IS NOT WHAT SHIPS**, and this is
     * the part that matters most.
     *
     * The test above enters through `storedSignerLeaf`. That proved the helper
     * agrees with the contract and proved nothing about
     * `AccountService.create` calling it: restoring the ed25519 line at
     * `src/core/account.ts` left **261 tests green**, including this file.
     * `core.test.ts` cannot see it, because it uses `leafCommitment` as an
     * opaque value on both sides of every approval — the writer and the
     * checker move together, which is the mirror's blind spot arriving at the
     * roster.
     *
  ...
  it('the adapter passes the SCOPE it is given, not the one it defaults to', () => {
    /*
     * **ONE LAYER BELOW WHERE A MIRROR CAN SEE.** Forcing
     * `src/midnight/commitments.ts` to ignore its `scope` argument and always
     * use `allVaults()` SURVIVED every test, because every caller in the suite
     * passes the default — so the coverage was of the default value and not of
     * the parameter.
     *
  ...
  it('the adapter is the circuit, so nothing restates the hash in TypeScript', () => {
    /*
     * A second implementation of a money rule is what this repository has been
     * caught by most often, and a test that then compares it to itself is
     * worth nothing. There is no second implementation: this asserts the
     * wrapper's ARGUMENTS and ENCODING, which is the only thing a mirror can
     * honestly hold.
     */
  ...
  it('the SIMULATED scheme still does not agree with the contract, and must not', () => {
    /*
     * The domain-separation rule, one argument further in.
     * `SimulatedCommitments` is a deliberately different HMAC scheme;
     * "unifying" the two would make the simulation emit values it cannot
     * prove, and the leaf is the one place that would look like progress.
     */
...
/**
 * **THE DEPLOYING DEVICE IS NOT A SIGNER.**
 *
 ...
 * constructor derived the seat from `x`, so the deploying device and the
 * seated signer were **the same value by construction** and no assertion could
 * tell them apart. The factory takes the founding leaf as its own argument now
 * — its only other one, since the threshold argument was deleted — so this
 * block can hand it two DIFFERENT devices, which is the only arrangement in
 * which "the deployer has no seat" is a claim that can be false.
 *
 ...
 */
describe('the deployer has no seat', () => {
  ...
  it('SEATS A LEAF THE PRODUCT WRITER PRODUCED, not one this test derived', async () => {
    /*
     * **THE COMPOSITION THAT ARRIVED WITH THE ARGUMENT, AND NOTHING ELSE
     * COVERED IT.**
     *
  ...
  it('still seats one signer, at a threshold of one, when founder and deployer are the same device', async () => {
     ...
     * TO GIVE ANY MORE.** The constructor takes one argument and sets
     * `threshold = 1`, so the pair below is the only state a constructor can
     * produce.
     */
...
/**
 * **AN ACCOUNT CANNOT BE FOUNDED WITH NO RULE, AND CANNOT BE FOUNDED WITH A
 * RULE NOBODY CAN MEET.**
 *
 ...
 * proposal no signer voted for. That is the first. And nothing bounded the
 * value from ABOVE either: an account founded needing fifty approvals with one
 * seat is frozen for ever, because `setThreshold` is the only way down and it
 * is itself behind `requireApproved`. That is the second.
 *
 * ── WHAT CHANGED, AND WHY THIS BLOCK LOOKS DIFFERENT NOW ─────────────────
 *
 * A FLOOR was built first — `assert(disclose(requiredApprovals) > 0, …)` — and
 * this block was written against it: hand the constructor a zero, watch it
 * refuse. **It was then decided that the argument goes instead of being
 * guarded.** `docs/company-accounts.md` §10.
 *
 ...
 *
 * The line that could still break is `threshold = 1;`. Break it to `threshold
 * = 0;` and the first defect is back, in full, with no argument anywhere near
 * it. **That is the mutation scored here**, and the test below is what goes
 * red for it: at a threshold of zero the approval gate passes with nobody
 * having approved, and the assertion is on a governed circuit REFUSING rather
 * than on any number read off the ledger.
 *
 ...
 * FOUNDING moment, and it sits beside the two leaf guards above, which are the
 * constructor's other arguments-are-attacker-chosen block.
 *
 * ── WHAT IT DOES NOT SHOW ────────────────────────────────────────────────
 *
 ...
 */
describe('an account is founded at one, and one is a rule', () => {
  ...
  it('founds every account at a threshold that is not met before anybody approves', async () => {
     ...
     * old block could not do. The constructor's floor made a zero unreachable
     * through the constructor, so `!(0 < 0)` PASSING was unexercisable by any
     * test, and this file said so plainly rather than claiming coverage it did
     * not have.
     *
     ...
     * PARAGRAPH OVERSTATED IT.** `setThreshold` runs `requireApproved` FIRST
     * and its payload check SECOND. `id` below was raised over `payload(31)`,
     * which is not a `setThresholdPayload`, so at a threshold of zero the
     * approval bar PASSES and the payload check then refuses with *"that
     * proposal is not for this threshold"*. The rule does not move. **What the
     * assertion reads is WHICH refusal came back**, and that is the
     * discrimination: at one it is
  ...
  it('founds every account with its threshold no higher than its seats', async () => {
    /*
     * **THE SECOND HALF, AND IT IS AN ASSERTION ABOUT A RELATIONSHIP RATHER
     * THAN A NUMBER.** An account whose threshold exceeds its seat count can
     * never approve anything, so it can never lower the threshold either —
     * frozen, with every vault that named it. The constructor's floor could
     * not have refused that: a floor bounds one end.
     *
     * One seat and a bar of one is the only state a constructor makes now, so
     * the relationship holds at birth. **The pair below is what
     * `setThreshold`'s ceiling then preserves for the rest of the account's
     * life** — the two together are why `signerLeaves.size() < threshold` is
     * unreachable, which is `docs/company-accounts.md` §10a and the reason
